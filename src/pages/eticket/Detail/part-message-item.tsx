...
import { Button, ScrollView, TabPanel, Tabs, TextArea, DropDownBox, List, ButtonGroup } from 'devextreme-react';
...
export const PartMessageItem = ({ data }: { data: EticketMessage }) => {
    ...
    const EmailDetail = ({ email }: { email: Email }) => {
        ...
        return (
            <div className="w-full pl-5 pr-5">
                ...
                <div className="flex pl-2 pt-1">
                    <span className="text-gray mr-2" style={{ width: "200px", display: "block" }}>Nội dung: </span>
    ...
    const InfoDetail = ({ data }: { data: EticketMessage }) => {
        if (data.Type == 'Email' && data.Email != null) return (<EmailDetail email={data.Email}></EmailDetail>);
        else if (data.Type == 'Call' && data.Call != null) return (<CallDetail call={data.Call}></CallDetail>);
        else if (data.Type == 'Event') return (<EventDetail data={data}></EventDetail>);
        //...
        return (<></>);
    }
    ...
    return (
        ...
        <div className={`w-full position-relative bg-white mb-3 pb-3 message-item ${data.Type.toLocaleLowerCase()}`}>

            {data.Type != "Event" ?
                <>
                    <div className="flex p-2">
                        <div className=" avatar-name mr-5">
                            <Avatar name={data.AuthorName} img={data.AuthorImage} size={"sx"} className="mr-1" />
                            <strong className="name">{data.AuthorName}</strong>
                        </div>
                        <span className="text-gray" style={{ lineHeight: "32px" }}>    <i className="dx-icon-clock mr-1"></i>{data.CreateDtime}</span>
