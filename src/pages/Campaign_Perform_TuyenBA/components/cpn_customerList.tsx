...
export const CpnCustomerList = ({ onItemSelected, onCallBtnClick }: { onItemSelected: any, onCallBtnClick: any }) => {
    ...
    const { data: campaignListData, isLoading: isLoadingCampaignList, refetch: refetchCampaignList } = useQuery(
    ...
    const campaignCustomerStatuses = useMemo(() => {
        return [
            ...
            { Code: "PENDING", Title: "Chưa thực hiện", icon: "dx-icon-tel", color: "red" },
            { Code: "DONE", Title: "Thành công", icon: "dx-icon-tel", color: "red" },
            { Code: "FAILED", Title: "Thực hiện cuộc gọi lỗi", icon: "dx-icon-tel", color: "red" },
            { Code: "NOANSWER", Title: "Đã gọi nhưng không nghe máy", icon: "dx-icon-tel", color: "red" },
            { Code: "CALLAGAIN", Title: "Hẹn gọi lại", icon: "dx-icon-tel", color: "red" },
            { Code: "NOANSWERRETRY", Title: "Đã gọi hết số lượt nhưng không nghe máy", icon: "dx-icon-tel", color: "red" },
            { Code: "DONOTCALL", Title: "Không liên hệ", icon: "dx-icon-tel", color: "red" },
            { Code: "FAILEDRETRY", Title: " Đã gọi hết số lượt nhưng cuộc gọi vẫn lỗi", icon: "dx-icon-tel", color: "red" },
    ...
    const [cpCustomerSearchConditions, setCpCustomerSearchConditions] = useState<any>({
        CampaignCode: null,
    ...
    const { data: campaignCustomerData, isLoading: isLoadingCampaignCustomer, refetch: refetchCampaignCustomer } = useQuery(
        ["Cpn_CampaignPerformPage", JSON.stringify(cpCustomerSearchConditions)],
        async () => {

            if (!!cpCustomerSearchConditions.CampaignCode) {
                const response = await api.Cpn_CampaignCustomer_Get(cpCustomerSearchConditions);
                if (response.isSuccess) {
    ...
    const CustomerItem = ({ item }: { item: Cpn_CampaignCustomerData }) => {
        ...
        return <div className="customer-item" >
            <div className="flex  cursor-pointer" onClick={() => { onItemSelected(item) }} >
                <Avatar img={item.CustomerAvatarPath} name={item.CustomerName} className={"mr-2"} />
                <div >
            ...
            {!!phoneNo && <Button icon="tel" className="btn-call" onClick={() => { onCallBtnClick(phoneNo); return false; }}></Button>}
        </div>
    }
        ...
        <ScrollView style={{ height: windowSize.height - 300 }} className="pt-1">
            ...
            <div className="w-full">
                {!!campaignCustomerData && !!campaignCustomerData.Data && <>
                    {campaignCustomerData?.Data.map((item: Cpn_CampaignCustomerData) => { return <CustomerItem key={item.CustomerCodeSys} item={item} /> })}
                </>}
            </div>
        </ScrollView>
    </div>
