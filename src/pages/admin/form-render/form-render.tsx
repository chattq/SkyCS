import Form, {IItemProps, Item, RequiredRule} from "devextreme-react/form";
import {useEffect, useMemo, useRef, useState} from "react";
import { match } from "ts-pattern";
import { RequiredField } from "@packages/common/Validation_Rules";
import { useClientgateApi } from "@packages/api";
import { MdMetaColGroupSpecDto, SearchParam } from "@packages/types";
import {CheckBox, LoadPanel, Switch, ValidationGroup, Validator} from "devextreme-react";
import { AdminContentLayout } from "@layouts/admin-content-layout";
import { useQuery } from "@tanstack/react-query";
import {nanoid} from "nanoid";
import {Validation} from "devextreme-react/gantt";
      ...
      .with("MASTERDATA", () => {
        return {
          ...
          onValueChanged: ({value, component}: any) => {
            const Name = datasourceMapping[field.ColCodeSys!].find((item: any) => item.Key === value)?.Value
    ...
    const items: IItemProps[] = customFields.map<IItemProps>((field) => {
      ...
      return {
        ...
        render: (param: any) => {
          ...
          // init data
          component.updateData(field.ColCode, options.filter((opt: any) => opt.IsSelected).map((opt: any) => opt.Value))
          return (
              ...
              <div className={'flex'}>
                <label className={`dx-field-item-label display-block pr-4`}>
                  <span className={'dx-field-item-label-content'}>
                    ...
                    {field.FlagIsNotNull === "1" && <span className={'dx-field-item-required-mark'}> *</span>}
                  </span>
                </label>
                        ...
                        <CheckBox
                          ...
                          onValueChanged={(e: any) => {
                            ...
                            if(value) {
                              ...
                              const index = data.findIndex((item: any) => item === opt.Value)
                              if(index > -1) {
                                data.splice(index, 1)
                              }
                            }
