...
export const notifApi = {
  searchNotification: async (networkId: string, params:any) => {
    ...
    const response = await callApiBase.post<Response<any>>("/Notification/SearchNotification", params, {
  ...
  markAsRead: async (networkId: string, params:any) => {
    ...
    const token = localStorage.getItem("token");
    const response = await callApiBase.post<Response<any>>("/Notification/MarkAsRead", {model: model}, {
      headers: { networkId: networkId }
    });
