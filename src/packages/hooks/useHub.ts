...
export const useHub = (url: string) => {
    ...
    useEffect(() => {
        ...
        if (connect) {
            const connect = new HubConnectionBuilder()
                ...
                .withUrl(`${base_signalr_url}/hub/global/?networkId=${auth.networkId}&orgId=${auth.orgData?.Id}&userId=${auth.currentUser?.Id}&userEmail=${auth.currentUser?.Email}`, { accessTokenFactory: () => auth.token ?? "" })
                //.withUrl(url)
        ...
        return () => {
            ...
            (<any>window).__hubEventList = (<any>window).__hubEventList.filter((h: any) => h.__id != uuid);
    ...
    const onReceiveMessage = (eventName: string, eventHandle: (data: any) => void) => {
